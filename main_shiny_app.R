...
server <- function(input, output, session) {
  ...
  observeEvent(input$select_button, {
    ...
    model.coef = paste0(
      sapply(names(coef),FUN = 
               function(x) paste(trim.space(toupper(x)),": ", round(coef[x],3), " (", round(se[x],2),")" ,sep = "")), collapse = ", ")
    
    model.params$Value <- paste0(
      "Transformation: <i>" , output$name, "</i></br>",
      "Model: <i>", output$model,"</i></br>",
      "Coefficients:</br>",
      "<i>", model.coef ,"</i></br>",
      "RMSE: <i>", round(as.numeric(output$RMSE),2) ,"</i>"
    )
